        ...
        for line in lines:
            ...
            if name in instr_dict:
                ...
                for key in instr_dict:
                    item = instr_dict[key]
                    if item["encoding"] == single_dict["encoding"] and same_base_ext(
                        ext_name, item["extension"]
                    ):
                        # disable different names with same encodings on the same base extensions
                        err_msg = f"instruction : {name} from "
                        err_msg += (
                            f"{ext_name} has the same encoding with instruction {key} "
                        )
                        err_msg += (
                            f'added from {item["extension"]} in same base extensions'
                        )
                        logging.error(err_msg)
                        raise SystemExit(1)
